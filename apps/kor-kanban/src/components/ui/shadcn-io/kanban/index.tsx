...
export const KanbanBoard = ({ id, children, className }: KanbanBoardProps) => {
  ...
  return (
    <div
      className={cn(
        'flex min-h-40 flex-col rounded-xl transition-colors duration-300',
        isOver ? 'bg-muted/30' : '',
        className
      )}
      ref={setNodeRef}
...
export const KanbanCard = ({
  ...
  return (
    <Card
      className={cn(
        'p-3 mb-3 md:mb-4 flex-col space-y-2 rounded-lg border border-border/40 bg-panel/40 shadow-sm backdrop-blur-sm transition-all duration-300 hover:shadow-md hover:border-primary/30 hover:bg-panel/60 group',
        isDragging && 'cursor-grabbing rotate-2 scale-105 shadow-xl ring-2 ring-primary z-50 opacity-90',
        isOpen && 'ring-1 ring-primary border-primary/50 bg-panel/80',
        className
      )}
      {...listeners}
      {...attributes}
...
export const KanbanHeader = (props: KanbanHeaderProps) => {
  ...
  return (
    <Card
      className={cn(
        'sticky top-0 z-20 flex shrink-0 items-center gap-2 p-3 mb-2 rounded-lg border border-border/50 backdrop-blur-xl bg-background/80 shadow-sm',
        props.className
      )}
      style={{
        backgroundImage: `linear-gradient(to right, hsl(var(${props.color}) / 0.1), transparent)`,
      }}
    >
...
export const KanbanProvider = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'inline-grid grid-flow-col auto-cols-[minmax(280px,360px)] gap-6 items-start min-h-full px-4 pb-4',
          className
        )}
      >
        {children}
